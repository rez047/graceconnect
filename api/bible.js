...
export default async function handler(req, res) {
  ...
  if (translation === 'swahili' || translation === 'swa' || translation === 'swv') {
    try {
      ...
      if (r.ok) {
        ...
        return res.status(200).json({ reference: (d.name || book + ' ' + chapter) + ' (Swahili)', verses: (d.verses || []).map(v => ({ verse: v.verse, text: v.text })) });
      }
    } catch (e) {}
  ...
  try {
    ...
    if (r.ok) {
      const d = await r.json();
      return res.status(200).json({ reference: d.reference, verses: (d.verses || []).map(v => ({ verse: v.verse, text: v.text })) });
    }
