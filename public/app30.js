...
// =====================================================
  // MY LIST EVERYWHERE → CURRENT UI (departments, ushirika, groups, categories)
  // =====================================================
  var _h32Cache = null, _h32CacheT = 0;
  async function h32MyCache(force) {
    if (!me()) return { list: [], byId: {} };
    if (!force && _h32Cache && Date.now() - _h32CacheT < 60000) return _h32Cache;
    var uid = me().id, list = [];
    ...
    async function fill(table, fk, rows, type) {
      if (!rows.length) return;
      var r = (await sb().from(table).select('id,name').in('id', rows.map(function (x) { return x[fk]; }))).data || [];
      var nm = {}; r.forEach(function (x) { nm[x.id] = x.name; });
      rows.forEach(function (x) { list.push({ type: type, id: x[fk], name: nm[x[fk]] || type, role: x.role || 'Member' }); });
    }
    await fill('departments', 'department_id', d, 'department');
    await fill('ushirikas', 'ushirika_id', u, 'ushirika');
    await fill('church_groups', 'group_id', g, 'group');
    if (c.length) {
      var cr = (await sb().from('church_group_categories').select('id,name').in('id', c.map(function (x) { return x.category_id; }))).data || [];
      var cn = {}; cr.forEach(function (x) { cn[x.id] = x.name; });
      c.forEach(function (x) { list.push({ type: 'category', id: x.category_id, name: cn[x.category_id] || 'Category', role: x.role || 'Member' }); });
    }
    var byId = {}; list.forEach(function (m) { byId[m.id] = m; });
    _h32Cache = { list: list, byId: byId }; _h32CacheT = Date.now();
    return _h32Cache;
  }
  var H32_OC = { department: 'h32OpenDept', ushirika: 'h32OpenUsh', group: 'h32OpenGrp', category: 'h32OpenCat' };
  var H32_ICON = { department: 'fa-briefcase', ushirika: 'fa-people-group', group: 'fa-users', category: 'fa-child' };
  var H32_GRAD = { department: 'var(--gradient-dept)', ushirika: 'var(--gradient-chat)', group: 'var(--gradient)', category: 'var(--gradient-warm)' };

  async function h32RebuildHomeList() {
    ...
    var html = cache.list.map(function (m) {
      return '<div class="my-dept-mini" style="background:' + H32_GRAD[m.type] + '" onclick="' + H32_OC[m.type] + '(\'' + m.id + '\')">'
        + '<div class="my-dept-mini-icon"><i class="fas ' + H32_ICON[m.type] + '"></i></div>'
        ...
        + '<div style="font-size:.58rem;opacity:.9;margin-top:4px;text-transform:uppercase;letter-spacing:.4px">' + m.type + '</div></div>';
    }).join('');
    html += '<div class="my-dept-join-more" onclick="h32OpenMyList()"><i class="fas fa-list" style="font-size:1.2rem;margin-bottom:6px"></i><div style="font-size:.75rem;font-weight:700">View Full List</div></div>';
    sc.innerHTML = html;
    sc.setAttribute('data-h32', '1');
    var el = document.getElementById('myDeptsCount'); if (el) el.textContent = cache.list.length + ' joined';
  }
  // re-run after the old renderer paints, so we always win
  if (window.renderMyDepts && !window.renderMyDepts._h32w) {
    var _rmd = window.renderMyDepts;
    window.renderMyDepts = function () { var res = _rmd.apply(this, arguments); setTimeout(h32RebuildHomeList, 60); return res; };
    window.renderMyDepts._h32w = true;
  }
  setInterval(function () {
    var home = document.getElementById('section-home');
    var sc = document.querySelector('.my-depts-scroll');
    if (me() && home && home.classList.contains('active') && sc && sc.getAttribute('data-h32') !== '1') h32RebuildHomeList();
  }, 4000);
  setInterval(function () { if (me()) h32MyCache(true).then(function () { var sc = document.querySelector('.my-depts-scroll'); if (sc && document.getElementById('section-home').classList.contains('active')) h32RebuildHomeList(); }); }, 60000);

  // rewire ANY stray home tile/onclick that still points at old openers or old sub-pages
  setInterval(async function () {
    if (!me()) return;
    var cache = await h32MyCache();
    document.querySelectorAll('#section-home [onclick]').forEach(function (el) {
      if (el.closest('.my-depts-scroll')) return;
      var oc = el.getAttribute('onclick') || '';
      if (oc.indexOf('h32Open') === 0 || oc.indexOf('h32OpenMyList') > -1) return;
      var m = oc.match(/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i);
      if (m && cache.byId[m[1]]) { el.setAttribute('onclick', H32_OC[cache.byId[m[1]].type] + '(\'' + m[1] + '\')'); return; }
      if (/openDepartment|openDept|openUshirika|openUsh|showSubPage\('home-mainDept|showSubPage\('home-dept|showSubPage\('home-ush/i.test(oc)) {
        var m2 = oc.match(/'([^']+)'/); if (m2 && cache.byId[m2[1]]) el.setAttribute('onclick', H32_OC[cache.byId[m2[1]].type] + '(\'' + m2[1] + '\')');
      }
    });
  }, 3000);

  // if an old sub-page ever becomes active, bounce straight into the current UI
  var OLD_PAGE_RE = /home-(mainDept|dept|department|ush|ushirika|mainUsh|group|mainGroup)/i;
  if (window.showSubPage && !window.showSubPage._h32w) {
    var _sp = window.showSubPage;
    window.showSubPage = function (p) { var res = _sp.apply(this, arguments); if (OLD_PAGE_RE.test(String(p))) setTimeout(function () { h32EscapeOld(p); }, 120); return res; };
    window.showSubPage._h32w = true;
  }
  async function h32EscapeOld(p) {
    var el = document.getElementById(p); if (!el || !el.classList.contains('active')) return;
    var cache = await h32MyCache();
    ...
    if (!hit) return;
    window[H32_OC[hit.type]](hit.id);
  }
  // hard-replace legacy opener names if they exist in older bundles
  [['openDepartment', 'department'], ['openDept', 'department'], ['openUshirika', 'ushirika'], ['openUsh', 'ushirika'], ['openGroup', 'group'], ['openCategory', 'category']].forEach(function (pair) {
    var fn = window[pair[0]];
    if (typeof fn === 'function' && !fn._h32r) {
      window[pair[0]] = function (id) { window[H32_OC[pair[1]]](id); };
      window[pair[0]]._h32r = true;
    }
  });

// =====================================================
  // FEATURED MEMBERS — system users added with the SAME code/fields as manual adds
  // =====================================================
  window._h32Fe = window._h32Fe || { uid: null, pic: null, roles: [] };

  window.h32FeatureSystemModal = async function () {
    if (!isAdmin()) return alert('Admin only.');
    var us = await users();
    window._h32Fe = { uid: null, pic: null, roles: [] };
    document.body.insertAdjacentHTML('beforeend',
      '<div class="modal-overlay show" id="h32FeM" style="display:flex" onclick="if(event.target===this)this.remove()"><div class="modal" onclick="event.stopPropagation()"><div class="modal-handle"></div>'
      + '<div class="modal-title"><i class="fas fa-star"></i> Add System Member to Featured</div>'
      + '<div id="h32FePick" style="max-height:260px;overflow-y:auto">' + us.map(function (u) {
          return '<div onclick="h32FePick(\'' + u.id + '\')" style="display:flex;gap:8px;align-items:center;padding:8px;border-radius:10px;background:var(--bg);margin-bottom:6px;cursor:pointer">' + avatarHtml(u, 32) + '<div style="flex:1"><b style="font-size:.85rem">' + esc(u.name) + '</b><div style="font-size:.68rem;color:var(--text-light)">' + esc(u.role || 'Member') + '</div></div><i class="fas fa-plus" style="color:var(--primary)"></i></div>';
        }).join('') + '</div>'
      + '<div id="h32FeForm" style="display:none"></div></div></div>');
  };

  window.h32FePick = async function (uid) {
    var us = await users(); var u = us.find(function (x) { return x.id === uid; }); if (!u) return;
    window._h32Fe = { uid: uid, pic: u.profile_pic || null, roles: (u.role ? [u.role] : ['Member']) };
    var sug = [];
    var d = (await sb().from('department_members').select('role').eq('user_id', uid)).data || [];
    var uu = (await sb().from('ushirika_members').select('role').eq('user_id', uid)).data || [];
    var g = (await sb().from('church_group_members').select('role').eq('user_id', uid)).data || [];
    var c = (await sb().from('church_group_category_members').select('role').eq('user_id', uid)).data || [];
    d.concat(uu, g, c).forEach(function (r) { var role = (r.role || 'Member'); if (sug.indexOf(role) === -1) sug.push(role); });
    document.getElementById('h32FePick').style.display = 'none';
    var f = document.getElementById('h32FeForm'); f.style.display = 'block';
    f.innerHTML =
      '<div style="display:flex;gap:10px;align-items:center;margin-bottom:10px">' + avatarHtml(u, 46) + '<div><b>' + esc(u.name) + '</b><div style="font-size:.7rem;color:var(--text-light)">' + esc(u.email || '') + '</div></div></div>'
      + '<div class="form-group"><label class="form-label">Name</label><input class="form-input" id="h32FeName" value="' + esc(u.name || '') + '"></div>'
      + '<div class="form-group"><label class="form-label">Position (roles to display — add / remove)</label><div id="h32FeChips" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px"></div>'
      + '<div style="display:flex;gap:6px"><input class="form-input" id="h32FeRoleAdd" placeholder="Add role / position..." style="flex:1"><button class="btn btn-primary btn-sm" onclick="h32FeAddRole()"><i class="fas fa-plus"></i></button></div>'
      + (sug.length ? '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:6px">' + sug.map(function (s) { return '<button class="chip chip-green" onclick="h32FeAddRole(\'' + esc(s) + '\')">' + esc(s) + '</button>'; }).join('') + '</div>' : '')
      + '</div>'
      + '<div class="form-group"><label class="form-label">Photo</label><div id="h32FePrev" style="margin-bottom:6px"></div>'
      + '<button class="btn btn-secondary btn-sm" onclick="h32FeUpload()"><i class="fas fa-camera"></i> ' + (u.profile_pic ? 'Change photo' : 'Upload photo') + '</button>'
      + (u.profile_pic ? ' <button class="btn btn-secondary-alt btn-sm" onclick="h32FeUsePic()"><i class="fas fa-user"></i> Use profile photo</button>' : '')
      + '</div>'
      + '<div class="form-group"><label class="form-label">Additional info (same field as outside-system members)</label><textarea class="form-textarea" id="h32FeInfo" rows="3" placeholder="Bio, vision, achievements... (optional)"></textarea></div>'
      + '<button class="btn btn-warm btn-block" onclick="h32SaveSystemFeatured()"><i class="fas fa-star"></i> Add to Featured</button>';
    h32FeRenderChips(); h32FeRenderPrev();
  };
  window.h32FeAddRole = function (r) {
    var v = (r || (document.getElementById('h32FeRoleAdd') || {}).value || '').trim(); if (!v) return;
    if (window._h32Fe.roles.indexOf(v) === -1) window._h32Fe.roles.push(v);
    var i = document.getElementById('h32FeRoleAdd'); if (i) i.value = '';
    h32FeRenderChips();
  };
  window.h32FeRemoveRole = function (i) { window._h32Fe.roles.splice(i, 1); h32FeRenderChips(); };
  function h32FeRenderChips() {
    var c = document.getElementById('h32FeChips'); if (!c) return;
    c.innerHTML = window._h32Fe.roles.map(function (r, i) { return '<span class="chip chip-green" style="font-size:.72rem">' + esc(r) + ' <b onclick="h32FeRemoveRole(' + i + ')" style="cursor:pointer;margin-left:4px;color:#991B1B">✕</b></span>'; }).join('') || '<span style="font-size:.7rem;color:var(--text-lighter)">No roles yet — add at least one position.</span>';
  }
  window.h32FeUpload = function () {
    var i = document.createElement('input'); i.type = 'file'; i.accept = 'image/*';
    i.onchange = async function () { var f = i.files && i.files[0]; if (!f) return; window._h32Fe.pic = await upload(f, 'featured'); h32FeRenderPrev(); };
    i.click();
  };
  window.h32FeUsePic = async function () { var us = await users(); var u = us.find(function (x) { return x.id === window._h32Fe.uid; }); window._h32Fe.pic = (u && u.profile_pic) || null; h32FeRenderPrev(); };
  function h32FeRenderPrev() {
    var p = document.getElementById('h32FePrev'); if (!p) return;
    p.innerHTML = window._h32Fe.pic ? '<img src="' + window._h32Fe.pic + '" style="width:64px;height:64px;border-radius:50%;object-fit:cover;display:block">' : '<div style="font-size:.7rem;color:var(--text-lighter)">No photo — initials will show.</div>';
  }
  window.h32SaveSystemFeatured = async function () {
    var fe = window._h32Fe;
    var name = (document.getElementById('h32FeName').value || '').trim();
    if (!name) return alert('Name required.');
    if (!fe.roles.length) return alert('Add at least one position/role to display.');
    var info = (document.getElementById('h32FeInfo').value || '').trim();
    // EXACT same payload shape as the working outside-system (manual) featured add
    var payload = {
      user_id: fe.uid,
      name: name,
      role: fe.roles.join(' • ') || 'Member',
      image_url: fe.pic || null,
      sort: (window._featured || []).length,
      additional_info: info || null
    };
    var r = await sb().from('featured_people').insert([payload]);
    if (r.error) return alert(r.error.message);
    document.getElementById('h32FeM').remove();
    alert('✅ Added to featured members.');
    if (window.loadFeatured) window.loadFeatured();
  };
  window.h32RemoveFeatured = async function (id) {
    if (!isAdmin() || !confirm('Remove this featured member?')) return;
    var r = await sb().from('featured_people').delete().eq('id', id);
    if (r.error) return alert(r.error.message);
    if (window.loadFeatured) window.loadFeatured();
  };
  // inject admin controls into the landing "Servants of God / Featured" section
  setInterval(function () {
    if (!isAdmin() || !me()) return;
    var landing = document.querySelector('.public-landing');
    if (!landing || landing.classList.contains('hidden')) return;
    var head = Array.from(landing.querySelectorAll('h1,h2,h3,h4,.section-title,.section-title-app'))
      .find(function (h) { return /servants of god|featured/i.test(h.textContent || ''); });
    if (!head) return;
    var sec = head.closest('section') || head.parentElement;
    if (!sec || sec.querySelector('[data-h32feadd]')) return;
    var b = document.createElement('button');
    b.className = 'btn btn-warm btn-sm'; b.setAttribute('data-h32feadd', '1');
    b.style.margin = '8px 0'; b.innerHTML = '<i class="fas fa-user-plus"></i> Add System Member to Featured';
    b.onclick = function () { window.h32FeatureSystemModal(); };
    sec.insertBefore(b, head.nextSibling);
    // per-card remove buttons for admin
    (window._featured || []).forEach(function (p) {
      if (!p || !p.id) return;
      var leaves = Array.from(sec.querySelectorAll('div,b,strong,h3,h4')).filter(function (dd) { return dd.children.length === 0 && (dd.textContent || '').trim() === String(p.name || '').trim(); });
      leaves.forEach(function (lf) {
        var card = lf.closest('.card,.ministry-card') || lf.parentElement;
        if (!card || card.querySelector('[data-h32fedel="' + p.id + '"]')) return;
        var t = document.createElement('button');
        t.className = 'btn btn-danger btn-sm'; t.setAttribute('data-h32fedel', p.id);
        t.style.cssText = 'position:absolute;top:6px;right:6px'; t.innerHTML = '<i class="fas fa-trash"></i>';
        t.onclick = function (ev) { ev.stopPropagation(); window.h32RemoveFeatured(p.id); };
        card.style.position = 'relative'; card.appendChild(t);
      });
    });
  }, 2000);
})();
