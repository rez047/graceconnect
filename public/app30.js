  ...
  async function h32CatMembers() {
    ...
    (m.data || []).forEach(function (r) {
      ...
      html += '<div class="card" style="margin-bottom:8px"><div style="display:flex;gap:10px;align-items:center">' + avatarHtml(u, 38)
        ...
        + (!self ? '<button class="btn btn-primary btn-sm" style="white-space:nowrap;margin-left:auto;" onclick="if(window.c26OpenChat){window.c26OpenChat(\'' + r.user_id + '\')}else if(window.h27ChatWith){window.h27ChatWith(\'' + r.user_id + '\')}else{alert(\'Chat not ready\')}"><i class="fas fa-comment-dots"></i> Chat</button>' : '') + '</div>'
...
/* ============================================================
   GRACECONNECT — GROUP MAIN FORUM FINAL DELETE FIX
   ------------------------------------------------------------
   Users:
     - Can delete ONLY their own posts/comments/replies.

   Admin:
     - Can delete ANY group-forum post/comment/reply.

   Important:
     - Does NOT touch media attachments.
     - Does NOT touch the comment composer.
     - Deletes child replies when a parent comment is deleted.
   ============================================================ */
(function () {
  'use strict';

  if (window.__GC_GROUP_FORUM_DELETE_FIX__) return;
  window.__GC_GROUP_FORUM_DELETE_FIX__ = true;
  ...
  function gcCanDelete(ownerId) {
    if (gcIsAdmin()) return true;
  ...
  /*
   * Delete a comment and all replies beneath it.
   */
  async function gcDeleteCommentTree(commentId) {
    ...
    if (!gcCanDelete(root.data.user_id)) {
      throw new Error('You can only delete your own comment.');
    }

    /*
     * Load every comment belonging to this post.
     */
    var all = await db
      .from('community_comments')
    ...
    function depth(id, seen) {
      ...
      return depth(String(row.parent_comment_id), seen) + 1;
    }
  ...
  /*
   * Delete a group forum post and all its comments/replies.
   */
  async function gcDeleteForumPost(postId) {
    ...
    if (post.data.group_type !== 'forum') {
      throw new Error('This is not a group forum post.');
    }

    if (!gcCanDelete(post.data.user_id)) {
    ...
    /*
     * Deepest replies first.
     */
    comments.sort(function (a, b) {
      return depth(String(b.id)) - depth(String(a.id));
    });
  ...
  /*
   * MAIN GROUP FORUM — DELETE POST
   */
  window.h30DelPost = async function (postId) {
    ...
    try {
      ...
      await gcDeleteForumPost(postId);

      gcNotify('Post deleted successfully.', 'success');

      if (typeof window.h30LoadForum === 'function') {
        await window.h30LoadForum();
        return;
      }

      if (typeof window.h30OpenForum === 'function') {
        await window.h30OpenForum();
        return;
      }

      if (typeof window.h30RenderForum === 'function') {
        await window.h30RenderForum();
      }

    } catch (error) {

      console.error(
        'GraceConnect group post deletion error:',
        error
      );
  ...
  /*
   * MAIN GROUP FORUM — DELETE COMMENT / REPLY
   */
  window.h30DelComment = async function (commentId) {
    ...
    try {
      ...
      await gcDeleteCommentTree(commentId);

      gcNotify('Comment deleted successfully.', 'success');
      ...
      console.error(
        'GraceConnect group comment deletion error:',
        error
      );
