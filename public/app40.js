...
/* ============================================================
GHOSTBUSTER v3: remove ONLY the empty-state ministry tag
Keep all real ministry cards untouched.
============================================================ */
...
/* Remove ghost bare TEXT nodes directly inside the grid */
Array.prototype.forEach.call(grid.childNodes, function (n) {
if (n.nodeType === 3 && GHOST.test((n.textContent || '').trim())) {
grid.removeChild(n);
}
});
}
...
clearTimeout(gbT);
gbT = setTimeout(cleanMinistriesGhost40, 200);
