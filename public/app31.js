...
// ============ BLOCK Z — FINAL UNIFICATION & FIXES (overrides every duplicate above) ============
(function () {
  function sbx() { return window.sb; }
  function meZ() { return window.user; }
  function isAdminZ() { return window.isAdmin ? window.isAdmin() : false; }
  function escZ(s) { return String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); }

  // ---- 1) ONE clean showSubPage ----
  window.showSubPage = function (id) {
    var hm = document.getElementById('home-main');
    if (hm && !hm.classList.contains('sub-page')) hm.classList.add('sub-page');
    document.querySelectorAll('.sub-page.active').forEach(function (p) { p.classList.remove('active'); });
    document.querySelectorAll('.section.active').forEach(function (s) { s.classList.remove('active'); });
    var el = document.getElementById(id); if (!el) return;
    var sec = el.closest('.section'); if (sec) sec.classList.add('active');
    var target = el.classList.contains('sub-page') ? el : el.closest('.sub-page');
    if (target) target.classList.add('active'); else el.classList.add('active');
    window._gcLastSub = id; window.scrollTo({ top: 0 });
    try {
  ...
  // ---- 2) ONE clean switchSection (clears c26 context BEFORE switching) ----
  window.switchSection = function (name) {
    if (window.C) { window.C.currentId = null; window.C.currentType = null; }
    document.querySelectorAll('.section.active').forEach(function (s) { s.classList.remove('active'); });
    var sec = document.getElementById('section-' + name); if (sec) sec.classList.add('active');
    var navs = document.querySelectorAll('.bottom-nav .nav-item');
    var map = { home: 0, ushirika: 1, dept: 2, groups: 3, discover: 4, event: 5, giving: 6 };
    navs.forEach(function (b) { b.classList.remove('active'); });
    if (navs[map[name]]) navs[map[name]].classList.add('active');
    setTimeout(function () {
      var s2 = document.getElementById('section-' + name); if (!s2) return;
      if (!s2.querySelector('.sub-page.active')) { var m = s2.querySelector('.sub-page'); if (m) m.classList.add('active'); }
      if (name === 'home') { var hm = document.getElementById('home-main'); if (hm && !hm.classList.contains('active')) hm.classList.add('active'); }
    }, 60);
    window.scrollTo({ top: 0 });
  };

  // ---- 3) Unified user deletion: RPC admin_delete_user first, profiles fallback ----
  async function zDeleteUser(userId) {
    try { var r = await sbx().rpc('admin_delete_user', { target_user_id: userId }); if (!r.error) return true; } catch (e) {}
    var r2 = await sbx().from('profiles').delete().eq('id', userId);
    return !r2.error;
  }
  window.deleteUserAccount = async function (userId, email) {
    if (!confirm('Permanently delete this user? This cannot be undone.')) return;
    var ok = await zDeleteUser(userId);
    alert(ok ? '✅ User deleted.' : '⚠️ Deletion failed.');
    try { if (window.openUserManagement) await window.openUserManagement(); } catch (e) {}
  };
  window.blockAndDeleteUser = async function (userId, email) {
    if (!email) return alert('No email on file.');
    if (!confirm('Delete user AND blocklist email? They cannot re-register.')) return;
    await zDeleteUser(userId);
    try {
      var ex = await sbx().from('email_blocklist').select('id').eq('email', email);
      if (!ex.data || !ex.data.length) await sbx().from('email_blocklist').insert([{ email: email, blocked_at: new Date().toISOString(), blocked_by: meZ() ? meZ().id : null }]);
    } catch (e) {}
    alert('✅ User deleted and email blocklisted.');
    try { if (window.openUserManagement) await window.openUserManagement(); } catch (e) {}
  };
  // Repoint the admin button to the SERVER-side modal (email_blocklist table)
  setInterval(function () {
    if (!isAdminZ() || !meZ()) return;
    var wrap = document.getElementById('gcUserMgmtBtn');
    if (wrap) { var b = wrap.querySelector('button'); if (b && /gcOpenUserManagement/.test(b.getAttribute('onclick') || '')) b.setAttribute('onclick', 'openUserManagement()'); }
  }, 3000);

  // ---- 4) Trivia: accept A–D / 1–4 on save + heal bad stored rows ----
  window.gcParseCorrect = function (val, n) {
    n = n || 4;
    var s = String(val == null ? '' : val).trim().toUpperCase();
    if (/^[A-Z]$/.test(s)) { var i = s.charCodeAt(0) - 65; if (i >= 0 && i < n) return i; }
    if (/^\d+$/.test(s)) { var v = parseInt(s, 10); if (v >= 1 && v <= n) return v - 1; if (v >= 0 && v < n) return v; }
    return -1;
  };
  document.addEventListener('click', function (e) {
    ...
    if (!(t.indexOf('add question') > -1 || t.indexOf('save question') > -1 || t.indexOf('save trivia') > -1)) return;
    ...
    if (!f) return;
    var p = window.gcParseCorrect(f.value, 4);
    if (p >= 0) f.value = String(p);
  }, true);
  var zHealed = false;
  setTimeout(async function () {
    if (zHealed || !window.sb) return; zHealed = true;
    try {
      var tbl = null, cands = ['trivia_questions', 'trivia', 'bible_trivia'];
      for (var i = 0; i < cands.length; i++) { var r = await sbx().from(cands[i]).select('id').limit(1); if (!r.error) { tbl = cands[i]; break; } }
      if (!tbl) return;
      var rows = (await sbx().from(tbl).select('*')).data || [];
      for (var j = 0; j < rows.length; j++) {
        var q = rows[j], opts = q.options || q.choices || [], n = 4;
        ...
        if (fixed !== null && fixed >= n) fixed = fixed - 1;
        if (fixed !== null && fixed >= 0 && fixed < n && fixed !== q.correct_index) await sbx().from(tbl).update({ correct_index: fixed }).eq('id', q.id);
      }
    } catch (e) {}
  }, 3000);

  // ---- 5) YouTube LIVE fix (live URLs play like normal videos) ----
  function zParseYT(url) {
    if (!url) return null;
    url = String(url).trim();
    if (/^[a-zA-Z0-9_-]{11}$/.test(url)) return { kind: 'video', id: url };
    var m = url.match(/(?:youtube\.com\/(?:watch\?(?:[^#]*&)?v=|embed\/|shorts\/|v\/|live\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
  ...
  if (window.startLive && !window.startLive._z) {
    window.startLive = async function () {
      var t = ((document.getElementById('liveTitle') || {}).value || '').trim();
      var u = ((document.getElementById('liveYouTube') || {}).value || '').trim();
      if (!t || !u) return alert('Title and YouTube URL required.');
      var p = zParseYT(u);
      if (!p) return alert('⚠️ Invalid YouTube link. Use watch?v=…, youtu.be/…, youtube.com/live/… or channel/UC… link.');
      var embed = zEmbed(p);
      try {
        if (window.sb) await sbx().from('live_sermons').insert([{ title: t, youtube_url: u, video_id: p.kind === 'video' ? p.id : null, embed_url: embed, is_live: true, started_at: new Date().toISOString(), started_by: meZ() ? meZ().id : null }]);
      } catch (e) {}
      var m = document.getElementById('liveModal'); if (m) m.classList.remove('show');
      var box = document.getElementById('sermonLiveBox');
      if (box) box.innerHTML = '<div style="font-weight:800;margin-bottom:8px">🔴 ' + escZ(t) + '</div>'
        + '<iframe src="' + embed + '" style="width:100%;aspect-ratio:16/9;border-radius:12px;border:none;display:block;background:#000" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>'
        + '<div style="text-align:center;margin-top:10px"><button class="btn btn-danger btn-sm" onclick="endLive()"><i class="fas fa-stop"></i> End Live</button></div>';
      alert('🔴 You are live!');
    };
    window.startLive._z = true;
  }
  if (window.postPreaching && !window.postPreaching._z) {
    var _ppZ = window.postPreaching;
    window.postPreaching = async function () {
      var el = document.getElementById('preachYouTube');
      if (el && el.value.trim()) {
        var p = zParseYT(el.value.trim());
        if (p && p.kind === 'video') el.value = 'https://www.youtube.com/watch?v=' + p.id;
        else if (p && p.kind === 'channel') el.value = 'https://www.youtube.com/embed/live_stream?channel=' + p.uc;
        else if (!p) return alert('⚠️ Could not read that YouTube link.');
      }
      return _ppZ.apply(this, arguments);
    };
    window.postPreaching._z = true;
  }
})();
})();
})();
