// public/app31.js — FINAL CLEAN REWRITE
(function () {
  console.log('✝️ app31.js — final clean module');
  ...
  async function users(force) { 
    ...
    var r = await sb().from('profiles').select('id,name,profile_pic,email,role').order('name'); 
  ...
  async function upload(file, path) { 
    ...
    var n = (path || 'media') + '/' + Date.now() + '_' + file.name; 
    var r = await sb().storage.from('media').upload(n, file); 
  ...
  function mediaHtml(url) { 
    ...
    return '<a href="' + url + '" target="_blank" class="btn btn-secondary-alt btn-sm" style="margin-top:6px"><i class="fas fa-paperclip"></i> File</a>'; 
  }
  ...
  window.gcAttach = function (key, labelId) { 
    var i = document.createElement('input'); i.type = 'file'; i.accept = '*/*'; 
    i.onchange = function () { var f = i.files && i.files[0]; if (!f) return; window._gcMedia[key] = f; var l = document.getElementById(labelId); if (l) l.innerHTML = '<i class="fas fa-check-circle"></i> ' + esc(f.name); }; 
    i.click(); 
  };

  (function () { 
    var s = document.createElement('style'); 
    s.textContent = '.btn-secondary{background:#EEF2FF;color:#3730A3;border:2px solid #C7D2FE;backdrop-filter:none}.public-landing .btn-secondary,.hero-section .btn-secondary{background:rgba(255,255,255,.2);color:#fff;border:2px solid rgba(255,255,255,.4);backdrop-filter:blur(10px)}'; 
    document.head.appendChild(s); 
  })();

  // ==========================================
  // 1. THE ONE TRUE showSubPage (Fixes Stacking)
  // ==========================================
  window.showSubPage = function (id) {
    document.querySelectorAll('.sub-page').forEach(function (sp) { sp.classList.remove('active'); });
    document.querySelectorAll('.section').forEach(function (s) { s.classList.remove('active'); });
    var el = document.getElementById(id); if (!el) return;
    var sec = el.closest('.section');
    if (sec) sec.classList.add('active');
    if (!el.classList.contains('sub-page')) el.classList.add('sub-page');
  ...
  // ==========================================
  // 2. FIX INBOX (switchSection wrapper)
  // ==========================================
  if (window.switchSection && !window.switchSection._gcfixed) {
    var _origSwitch = window.switchSection;
    window.switchSection = function (name) {
      document.querySelectorAll('.sub-page').forEach(function (sp) { sp.classList.remove('active'); });
      document.querySelectorAll('.section').forEach(function (s) { s.classList.remove('active'); });
      var res = _origSwitch.apply(this, arguments);
      if (name === 'home') {
        var hm = document.getElementById('home-main');
        if (hm) {
          if (!hm.classList.contains('sub-page')) hm.classList.add('sub-page');
  ...
  // ==========================================
  // 3. FIX FORUM/PRAYER/PLANS (Hide sub-pages when opening full pages)
  // ==========================================
  function gcWrapPage(name) {
    var fn = window[name];
    if (typeof fn !== 'function' || fn._gcwrapped) return;
    window[name] = function () {
      document.querySelectorAll('.sub-page').forEach(function (sp) { sp.classList.remove('active'); });
      document.querySelectorAll('.section').forEach(function (s) { s.classList.remove('active'); });
      return fn.apply(this, arguments);
    };
    window[name]._gcwrapped = true;
  }
  setInterval(function () {
    ['h28OpenForum', 'h28OpenPrayer', 'h30OpenPlans', 'h27OpenPage'].forEach(gcWrapPage);
  }, 1500);

  // ==========================================
  // 4. INJECT NEW QUICK TILES (Forum, Plans, Prayer Wall)
  // ==========================================
  setInterval(function () {
    var grid = document.querySelector('#home-main .grid-3'); 
    if (!grid || grid.querySelector('[data-gc-forum]')) return;
    var forumTile = '<div class="mini-card mc-blue" data-gc-forum="1" onclick="if(window.h28OpenForum)window.h28OpenForum();else if(window.h27OpenPage)window.h27OpenPage(\'forum\')"><i class="fas fa-comments"></i><div class="mc-title">Public Forum</div><div class="mc-sub">Community</div></div>';
    var plansTile = '<div class="mini-card mc-gold" data-gc-plans="1" onclick="if(window.h30OpenPlans)window.h30OpenPlans();else if(window.h27OpenPage)window.h27OpenPage(\'plans\')"><i class="fas fa-calendar-check"></i><div class="mc-title">Plans</div><div class="mc-sub">Goals</div></div>';
    var prayerTile = '<div class="mini-card mc-green" data-gc-prayer="1" onclick="if(window.h28OpenPrayer)window.h28OpenPrayer();else if(window.h27OpenPage)window.h27OpenPage(\'prayer\')"><i class="fas fa-hands-praying"></i><div class="mc-title">Prayer Wall</div><div class="mc-sub">Community</div></div>';
    grid.insertAdjacentHTML('beforeend', forumTile + plansTile + prayerTile);
  }, 2000);
  ...
  setInterval(function () { 
    try {
      ...
      if (eds.length > 1) { for (var i = 1; i < eds.length; i++) { if (eds[i].parentNode) eds[i].parentNode.removeChild(eds[i]); } var b0 = document.querySelector('[data-h30edit]'); var ban = b0 && b0.previousElementSibling; if (b0 && ban && ban.classList && ban.classList.contains('dept-banner') && !ban.querySelector('[data-h30edit]')) ban.appendChild(b0); }
  ...
  async function gcMyCache(force) { 
    if (!me()) return { list: [], byId: {} }; 
    if (!force && _gcCache && Date.now() - _gcCacheT < 60000) return _gcCache; 
    var uid = me().id, list = [];
  ...
  window.gcOpenDept = function (id) { if (window.c26OpenGroup) window.c26OpenGroup('department', id); else if (window.c26OpenHome) window.c26OpenHome('department'); };
  window.gcOpenUsh = function (id) { if (window.c26OpenGroup) window.c26OpenGroup('ushirika', id); else if (window.c26OpenHome) window.c26OpenHome('ushirika'); };
  window.gcOpenGrp = function (id) { if (window.ggOpenGroup) window.ggOpenGroup(id); };
  window.gcOpenCat = function (id) { window.gcOpenCategoryFull(id); };
  
  window.gcOpenMyList = async function () { 
    if (!me()) return alert('Please log in first.'); 
    var sec = document.getElementById('section-gclist'); 
    if (!sec) { sec = document.createElement('div'); sec.id = 'section-gclist'; sec.className = 'section'; sec.innerHTML = '<div id="gclist-root" class="sub-page active"></div>'; (document.querySelector('main') || document.body).appendChild(sec); } 
    document.querySelectorAll('.section').forEach(function (s) { s.classList.remove('active'); }); 
    sec.classList.add('active'); 
    document.querySelectorAll('.bottom-nav .nav-item').forEach(function (b) { b.classList.remove('active'); }); 
    document.getElementById('gclist-root').innerHTML = '<button class="back-btn" onclick="gcListBack()"><i class="fas fa-arrow-left"></i> Back</button><div style="border-radius:20px;padding:16px;color:#fff;background:linear-gradient(135deg,#8B5CF6,#EC4899);font-weight:800;font-size:1.15rem;margin-bottom:14px"><i class="fas fa-list"></i> My List</div><div id="gclist-body"><div class="card">Loading...</div></div>'; 
    window.scrollTo({ top: 0 }); gcLoadMyList(); 
  };
  
  window.gcListBack = function () { 
    document.querySelectorAll('.section').forEach(function (s) { s.classList.remove('active'); }); 
    var h = document.getElementById('section-home'); if (h) h.classList.add('active'); 
    var n = document.querySelectorAll('.bottom-nav .nav-item'); if (n[0]) n[0].classList.add('active'); 
    document.querySelectorAll('#section-home .sub-page').forEach(function (p) { p.classList.remove('active'); }); 
    var hm = document.getElementById('home-main'); if (hm) hm.classList.add('active'); 
    window.scrollTo({ top: 0 }); 
  };
  
  async function gcLoadMyList() { 
  ...
  async function gcRebuildStrip() { 
    ...
    sc.innerHTML = html; sc.setAttribute('data-gc', '1'); gcUpdateCount(cache.list.length); 
  }
  
  if (window.renderMyDepts && !window.renderMyDepts._gcw) { 
    var _rd = window.renderMyDepts; 
    window.renderMyDepts = function () { var r = _rd.apply(this, arguments); setTimeout(gcRebuildStrip, 60); return r; }; 
    window.renderMyDepts._gcw = true; 
  }
  ...
  window.gcManageFeatured = async function () { 
    if (!isAdmin()) return alert('Admin only.'); 
    if (window.loadFeatured) { try { await window.loadFeatured(); } catch (e) {} } 
    var list = window._featured || []; 
  ...
  window.gcFeatureSystemModal = async function () { 
    if (!isAdmin()) return alert('Admin only.'); 
    var us = await users(); 
    window._gcFe = { uid: null, pic: null, roles: [] }; 
  ...
  window.gcFePick = async function (uid) { 
    var us = await users(); var u = us.find(function (x) { return x.id === uid; }); if (!u) return; 
    window._gcFe = { uid: uid, pic: u.profile_pic || null, roles: u.role ? [u.role] : ['Member'] }; 
    var sug = []; 
  ...
  // Tap-to-add Featured Form
  window._gcFeEdit = { uid: null, pic: null, roles: [] };
  if (window.feAdd21 && !window.feAdd21._gcw) { window.feAdd21 = function (uid) { window.gcFeOpenForm(uid); }; window.feAdd21._gcw = true; }
  ...
  window.gcOpenCategoryFull = async function (categoryId) { 
    if (!me()) return alert('Please log in first.'); 
    var sec = document.getElementById('section-gccat'); 
    if (!sec) { sec = document.createElement('div'); sec.id = 'section-gccat'; sec.className = 'section'; sec.innerHTML = '<div id="gccat-root" class="sub-page active"></div>'; (document.querySelector('main') || document.body).appendChild(sec); } 
    document.querySelectorAll('.section').forEach(function (s) { s.classList.remove('active'); }); 
    sec.classList.add('active'); 
    document.querySelectorAll('.bottom-nav .nav-item').forEach(function (b) { b.classList.remove('active'); }); 
    var root = document.getElementById('gccat-root'); root.innerHTML = '<div class="card">Loading...</div>'; 
    window.scrollTo({ top: 0 });
  ...
  async function gcCatLoadPosts() { 
    ...
    var us = await users(); window._gcUsers = us; 
...
// ==========================================
  // 11. BLOCK H v2 — white-screen fix, trio removal, 50% taps, SPEED
  // ==========================================
  // (a) FIX WHITE SCREEN: ensure every section always has an active sub-page
  (function () {
    var cur = window.switchSection; if (!cur || cur._gcfix2) return;
    window.switchSection = function (name) {
      var r = cur.apply(this, arguments);
      setTimeout(function () {
        var sec = document.getElementById('section-' + name);
        if (sec && sec.classList.contains('active') && !sec.querySelector('.sub-page.active')) {
          var first = sec.querySelector('.sub-page'); if (first) first.classList.add('active');
        }
        window.scrollTo({ top: 0 });
      }, 25);
      return r;
    };
    window.switchSection._gcfix2 = true;
  })();

  // (b) REMOVE OLD TRIO (match by title text, any card-like element, never mini-cards)
  function gcRemoveOldTrio() {
    var host = document.getElementById('home-main'); if (!host) return;
    var titles = ['Prayer Wall', 'Public Forum', 'Plans'];
    Array.prototype.slice.call(host.querySelectorAll('div,a,button')).forEach(function (el) {
      if (el.classList.contains('mini-card') || el.closest('.mini-card') || el.closest('.my-depts-scroll') || el.closest('.grid-3')) return;
      var t = (el.textContent || '').replace(/\s+/g, ' ').trim();
      var hit = titles.some(function (x) { return t === x || (t.indexOf(x) === 0 && t.length <= x.length + 12); });
      if (!hit) return;
      var cls = String(el.className || ''), sty = el.getAttribute('style') || '';
      if (/card|gradient|tile/i.test(cls) || /gradient/i.test(sty)) {
        var grid = el.parentElement; el.remove();
        if (grid && !grid.children.length) grid.remove();
      }
    });
  }
  setInterval(gcRemoveOldTrio, 1500); setTimeout(gcRemoveOldTrio, 400);

  // (c) FIX 50% TAPS: delegated listener (single fire, survives DOM churn)
  (function () {
  ...
  // (d) SPEED: parallel cache (2 round trips total) + instant badge
  async function gcMyCacheFast(fresh) {
    if (!me()) return { list: [], byId: {} };
    if (!fresh && _gcCache && Date.now() - _gcCacheT < 30000) return _gcCache;
    var uid = me().id;
    var ms = await Promise.all([
      sb().from('department_members').select('*').eq('user_id', uid),
      sb().from('ushirika_members').select('*').eq('user_id', uid),
  ...
  async function gcFastCount() {
    ...
    var el = document.getElementById('myDeptsCount'); if (el) el.textContent = (res[0] + res[1] + res[2] + res[3]) + ' joined';
  }

  // (e) instant side-scroll strip
  async function gcInstantStrip() {
    var sc = document.querySelector('.my-depts-scroll'); if (!sc || !me()) return;
    gcFastCount();
    var cache = await gcMyCacheFast(false);
    var html = cache.list.map(function (m) { return '<div class="my-dept-mini" style="background:' + GC_GRAD[m.type] + '" onclick="' + GC_OC[m.type] + '(\'' + m.id + '\')"><div class="my-dept-mini-icon"><i class="fas ' + GC_ICON[m.type] + '"></i></div><div class="my-dept-mini-name">' + esc(m.name) + '</div><div style="margin-top:6px"><span class="my-dept-mini-role-badge">' + esc(m.role) + '</span></div><div style="font-size:.58rem;opacity:.9;margin-top:4px;text-transform:uppercase">' + m.type + '</div></div>'; }).join('');
    html += '<div class="my-dept-join-more" onclick="gcOpenMyList()"><i class="fas fa-list" style="font-size:1.2rem;margin-bottom:6px"></i><div style="font-size:.75rem;font-weight:700">View Full List</div></div>';
    sc.innerHTML = html; sc.setAttribute('data-gc', '1');
  }
  setTimeout(function () { gcInstantStrip(); }, 250);
  setInterval(function () { if (!me()) return; var sc = document.querySelector('.my-depts-scroll'); if (sc && sc.getAttribute('data-gc') !== '1') gcInstantStrip(); }, 1500);
  setInterval(function () { if (me() && !document.hidden) gcFastCount(); }, 20000);
  document.addEventListener('visibilitychange', function () { if (!document.hidden && me()) { gcFastCount(); gcInstantStrip(); } }, { passive: true });

  // (f) FAST MY LIST PAGE: render from cache instantly, refresh in background
  function gcRenderList(cache) {
    var box = document.getElementById('gclist-body'); if (!box) return;
    var T = { department: '🏛️ My Departments', ushirika: '🏘️ My Ushirika', group: '👥 My Groups', category: '🧒 My Categories' };
    var html = '';
    ['department', 'ushirika', 'group', 'category'].forEach(function (tp) {
  ...
  window.gcOpenMyList = async function () {
    ...
    sec.classList.add('active');
    document.querySelectorAll('.bottom-nav .nav-item').forEach(function (b) { b.classList.remove('active'); });
    document.getElementById('gclist-root').innerHTML = '<button class="back-btn" onclick="gcListBack()"><i class="fas fa-arrow-left"></i> Back</button><div style="border-radius:20px;padding:16px;color:#fff;background:linear-gradient(135deg,#8B5CF6,#EC4899);font-weight:800;font-size:1.15rem;margin-bottom:14px"><i class="fas fa-list"></i> My List</div><div id="gclist-body"></div>';
    window.scrollTo({ top: 0 });
    gcRenderList(await gcMyCacheFast(false));   // instant (cache or parallel fetch)
    gcRenderList(await gcMyCacheFast(true));    // then fresh data
  };

  // (g) PREFETCH everything at boot + idle so no section ever shows long "Loading..."
  var GC_LOADERS = ['loadCauses', 'loadGiving', 'loadEvents', 'loadChatInbox', 'loadChats', 'loadFeatured', 'loadLeaders', 'loadNotifs', 'loadDocuments', 'loadPreachings', 'loadPlans', 'loadPrayerWall', 'loadDiscover'];
  function gcPrefetch() { GC_LOADERS.forEach(function (n) { try { if (typeof window[n] === 'function') window[n](); } catch (e) {} }); gcMyCacheFast(true); }
  var _gcPf = false;
  function gcPrefetchOnce() { if (_gcPf || !me()) return; _gcPf = true; gcPrefetch(); }
  setTimeout(gcPrefetchOnce, 600);
  setInterval(gcPrefetchOnce, 1200);
  if ('requestIdleCallback' in window) { requestIdleCallback(function () { gcPrefetchOnce(); }, { timeout: 2500 }); }
