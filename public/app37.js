...
(function () {
  ...
  function patchCategory37() {
    ...
    /*
     * Category Members already renders each member as a .card.
     * The important part is that the user's ID is present in the
     * existing inline handlers generated by h32CatMembers().
     *
     * Do NOT depend on profiles/name matching.
     * Do NOT rebuild the member card.
     * We only add the missing Chat button to the existing card.
     */
    ...
    Array.prototype.forEach.call(cards, function (card) {
        ...
        if (hasChat37(card) || hasChatText37(card)) {
            return;
        }
        ...
        /*
         * h32CatMembers() already contains:
         * c26OpenChat('USER_ID')
         * or h27ChatWith('USER_ID')
         */
        var uid = uidFromHtml37(html);
        ...
        /*
         * Put the button inside the existing card's action row
         * when one exists. Otherwise append to the card.
         */
        var actionRow =
            card.querySelector(
                '.d-flex, .btn-group, .actions, [style*="display:flex"]'
            );

        var button = makeChatBtn37(uid);
