...
// ═══════════ app20-append: FINAL — kills flicker wars, one static dept widget, Bible all translations, chat inbox list ═══════════
(function(){
  function _g(id){return _realGet(id);}
  var _realGet=Function.prototype.call.bind(document.getElementById,document);

  // 1) Stop legacy writers (app16/app17) forever: their box ids now return a detached dummy
  window._gcDummyMeet=document.createElement('div');
  document.getElementById=function(id){
    if(id==='deptWeekMeet14'||id==='deptWeekMeet17')return window._gcDummyMeet;
    return _realGet(id);
  };

  // 2) Kill EVERY existing interval (ends the redraw wars), then re-add only essential loops
  var _max=setInterval(function(){},999999);clearInterval(_max);
  for(var i=1;i<=_max;i++){try{clearInterval(i);}catch(e){}}

  setInterval(function(){ // landing-page injections (app8)
    try{
      if(typeof injectLoginBtn==='function')injectLoginBtn();
      if(typeof injectUshirikaEditBtn==='function')injectUshirikaEditBtn();
      if(typeof addUshirikaClickHandlers==='function')addUshirikaClickHandlers();
      if(typeof injectArticlesSection==='function')injectArticlesSection();
      if(typeof injectArticleModal==='function')injectArticleModal();
      if(typeof injectLocationsAndSocials==='function')injectLocationsAndSocials();
    }catch(e){}
  },2000);
  setInterval(function(){ // app10 UI sweep + stray-card removal (DOM-only)
    try{
      bindDeptCards();bindUshCards();ensureUshDetailPage();injectDeptTabMeetBtn();injectDeptAddRoleBtn();
      sweepMeetingButtons();enhanceRoleModals();
      var host=_g('home-mainDept');
      if(host){
        var keep=_g('deptWeekMeet18');
        var cards=host.querySelectorAll('.card,.card-cool');
        for(var c=0;c<cards.length;c++){
          var el=cards[c];
          if(/This Week'?s Meeting/i.test(el.textContent||'')&&!(keep&&keep.contains(el)))el.style.display='none';
        }
        if(keep)keep.style.display='';
      }
    }catch(e){}
  },2000);
  setInterval(function(){ // memberships / counts / pending
    try{
      if(user){loadMyMemberships9();loadMemberCounts9();loadMyDepts();}
      if(isAdmin()&&typeof loadPending==='function')loadPending();
    }catch(e){}
  },10000);
  setInterval(function(){ // greeting clock
    try{
      var t=document.querySelector('.greeting-time');
      if(t){var n=new Date();t.textContent=n.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'})+' • '+n.toLocaleTimeString('en-US');}
    }catch(e){}
  },1000);

  // remove legacy boxes + static blue card once now
  (function(){
    ['deptWeekMeet14','deptWeekMeet17'].forEach(function(id){var e=_realGet(id);if(e&&e.parentNode)e.parentNode.removeChild(e);});
    var host=_g('home-mainDept');
    if(host){var cards=host.querySelectorAll('.card,.card-cool');
      for(var c=0;c<cards.length;c++){var el=cards[c];
        if(/This Week'?s Meeting/i.test(el.textContent||'')&&!(_realGet('deptWeekMeet18')&&_realGet('deptWeekMeet18').contains(el)))el.style.display='none';}}
  })();

  // 3) THE single department widget (white card, static, event-driven)
  function inner20(m,can,deptId){
    var h='<div class="section-title-app" style="margin-bottom:6px"><i class="fas fa-calendar-day"></i> This Week\'s Meeting</div>';
    if(!m)h+='<div style="text-align:center;padding:14px;color:var(--text-lighter)">No meeting scheduled yet</div>';
  ...
  function calmDept20(deptId,force){
    try{
      if(!sb||!deptId)return;
      var host=_g('home-mainDept');if(!host)return;
      var box=_realGet('deptWeekMeet18');
      ...
      sb.from('weekly_meetings').select('*').eq('department_id',deptId).order('created_at',{ascending:false}).limit(1).then(function(r){
        ...
        if(!force&&box.dataset.sig20===sig){box.style.display='';return;}
        box.dataset.sig20=sig;if(m)window._curMeetingId=m.id;
        box.innerHTML='<div class="card" style="margin-bottom:14px">'+inner20(m,can,deptId)+'</div>';
        box.style.display='';
        var cards=host.querySelectorAll('.card,.card-cool');
        for(var c=0;c<cards.length;c++){var el=cards[c];if(/This Week'?s Meeting/i.test(el.textContent||'')&&!box.contains(el))el.style.display='none';}
      });
    }catch(e){}
  }
  window.delDeptMeet20=function(mid,deptId){
    deptId=deptId||currentDeptId;
    if(!mid||!isDeptLeader9(deptId))return alert('🚫 Leader/admin only.');
    if(!confirm('Delete this meeting?'))return;
    sb.from('weekly_meetings').delete().eq('id',mid).then(function(r){
      if(r.error)return alert('⚠️ '+r.error.message);
      alert('✅ Deleted');window._curMeetingId=null;calmDept20(deptId,true);
    });
  };
  var _o20=window.openDeptForum;
  window.openDeptForum=function(id){var r=_o20?_o20.apply(this,arguments):undefined;
    setTimeout(function(){calmDept20(id,true);},1200);setTimeout(function(){calmDept20(id,true);},2500);return r;};
  var _s20=window.saveDeptMeeting9;
  window.saveDeptMeeting9=function(){var r=_s20?_s20.apply(this,arguments):undefined;
    var d=(_realGet('dm9Pick')||{value:currentDeptId}).value;setTimeout(function(){calmDept20(d,true);},1500);return r;};
  var _u20=window.updateMeeting;
  window.updateMeeting=function(){var r=_u20?_u20.apply(this,arguments):undefined;
    setTimeout(function(){calmDept20(currentDeptId,true);},1500);return r;};

  // 4) Bible: ALL translations, 4-layer chain (own proxy → direct → two CORS relays)
  var BOOKS20=["Genesis","Exodus","Leviticus","Numbers","Deuteronomy","Joshua","Judges","Ruth","1 Samuel","2 Samuel","1 Kings","2 Kings","1 Chronicles","2 Chronicles","Ezra","Nehemiah","Esther","Job","Psalm","Proverbs","Ecclesiastes","Song of Solomon","Isaiah","Jeremiah","Lamentations","Ezekiel","Daniel","Hosea","Joel","Amos","Obadiah","Jonah","Micah","Nahum","Habakkuk","Zephaniah","Haggai","Zechariah","Malachi","Matthew","Mark","Luke","John","Acts","Romans","1 Corinthians","2 Corinthians","Galatians","Ephesians","Philippians","Colossians","1 Thessalonians","2 Thessalonians","1 Timothy","2 Timothy","Titus","Philemon","Hebrews","James","1 Peter","2 Peter","1 John","2 John","3 John","Jude","Revelation"];
  function bookNum20(b){for(var i=0;i<BOOKS20.length;i++){if(BOOKS20[i].toLowerCase()===String(b).toLowerCase())return i+1;}return 1;}
  function fetchChapter20(trans,book,chapter){
    var isSw=(trans==='Swahili');
    var code=({KJV:'kjv',NKJV:'kjv',NIV:'niv',WEB:'web',ASV:'asv',YLT:'ylt',DARBY:'darby',DRA:'dra'})[trans]||'kjv';
    var direct=isSw?('https://api.getbible.net/v2/swahili/'+bookNum20(book)+'/'+chapter+'.json')
                   :('https://bible-api.com/'+encodeURIComponent(book+' '+chapter)+'?translation='+code);
    function norm(d){var vs=(d&&d.verses)?d.verses:[];return{reference:d.reference||d.name||(book+' '+chapter),verses:vs.map(function(v){return{verse:v.verse,text:v.text};})};}
  ...
  window.loadBibleChapter=function(){
    var trans=(_realGet('readerTrans')||{}).value||'KJV';
    var ref=(_realGet('readerRef')||{value:'Genesis 1'}).value.trim();
    var parts=ref.match(/^(.+?)\s+(\d+)$/);
    var book=parts?parts[1]:'Genesis';var ch=parts?parts[2]:1;
    var out=_realGet('readerOut');if(!out)return;
    out.innerHTML='<div style="color:#94A3B8">Loading '+esc(trans)+'...</div>';
    fetchChapter20(trans,book,ch).then(function(d){
      if(!d||!d.verses||!d.verses.length){out.innerHTML='<div style="color:#991B1B">Not found in '+esc(trans)+'. Try "John 3" or "Psalm 23".</div>';return;}
      ...
      out.innerHTML=h;
    }).catch(function(){out.innerHTML='<div style="color:#991B1B">Could not load '+esc(trans)+' — check connection and retry.</div>';});
  };

  // 5) Start New Chat list: tap row or Inbox button opens private chat
  var _om20=window.openModal;
  window.openModal=function(id){
    var r=_om20?_om20.apply(this,arguments):undefined;
    if(id==='newChatModal'){
      setTimeout(function(){
        var pk=_realGet('newChatPicker');if(!pk||!user)return;
        ...
        pk.innerHTML=list.map(function(u2){
          return '<div class="user-pick-item" onclick="openChatWith(\''+u2.id+'\')" style="cursor:pointer">'+
            ...
            '<div style="flex:1"><div style="font-weight:600">'+esc(u2.name)+'</div><div style="font-size:.7rem;color:var(--text-light)">'+esc(u2.role||'member')+'</div></div>'+
...
console.log('✝️ app20-append active (static dept widget + bible chain + chat inbox)');
