...
/* ============================================================
   ADDITIVE PATCH — CATEGORY CHAT INBOX
   Adds Chat Inbox button/function to category subsection.
   Uses same logic as existing group chat inbox where possible.
   Paste at the very bottom of public/app35.js.
   ============================================================ */

(function () {
    'use strict';

    console.log('✝️ GraceConnect CATEGORY CHAT INBOX PATCH loaded');

    /* =========================================================
       CHAT INBOX LOGIC
       Reuses group logic if it exists.
    ========================================================= */

    function OPEN_CATEGORY_CHAT_INBOX() {
        try {

            /*
              1. If the group section already has a visible Chat Inbox
              button, click it so the exact same logic is used.
            */
            var groupButtonSelectors = [
                '[data-group-chat-inbox]',
                '[data-chat-inbox]',
                'button[onclick*="openChatInbox"]',
                'button[onclick*="Open Chat Inbox"]',
                'button[onclick*="showSubPage(\'messages\')"]',
                'button[onclick*="switchSection(\'messages\')"]'
            ];

            for (var i = 0; i < groupButtonSelectors.length; i++) {
                var groupButton =
                    document.querySelector(
                        groupButtonSelectors[i]
                    );
            ...
            /*
              2. If global chat inbox functions exist, use them.
            */
            if (typeof window.openChatInbox === 'function') {
                window.openChatInbox();
                return;
            }

            if (typeof window.openGroupChatInbox === 'function') {
                window.openGroupChatInbox();
                return;
            }

            if (typeof window.ggOpenChatInbox === 'function') {
                window.ggOpenChatInbox();
                return;
            }

            if (typeof window.showChatInbox === 'function') {
                window.showChatInbox();
                return;
            }
            ...
            /*
              3. Standard GraceConnect navigation fallbacks.
            */
            if (typeof window.showSubPage === 'function') {
                window.showSubPage('messages');
                return;
            }
            ...
            /*
              4. Existing chat helpers from earlier category/group code.
            */
            if (typeof window.c26OpenChat === 'function') {
                window.c26OpenChat();
                return;
            ...
            if (typeof window.h27ChatWith === 'function') {
                window.h27ChatWith(null);
                return;
            }

            /*
              5. Bottom navigation fallback.
            */
            var navSelectors = [
                '.bottom-nav [onclick*="messages"]',
                '.bottom-nav [data-tab="messages"]',
                '[data-section="messages"]',
                '[onclick*="showSubPage(\'messages\')"]',
                '[onclick*="switchSection(\'messages\')"]'
            ];

            for (var n = 0; n < navSelectors.length; n++) {
                var navButton =
                    document.querySelector(
                        navSelectors[n]
                    );
            ...
            console.error(
                'Category chat inbox error:',
                e
            );
        }

        alert('Chat inbox is not available.');
    }

    /*
      Expose globally so HTML buttons can also use:
      onclick="openCategoryChatInbox()"
    */
    window.openCategoryChatInbox =
        OPEN_CATEGORY_CHAT_INBOX;

    window.h32CategoryChatInbox =
        OPEN_CATEGORY_CHAT_INBOX;

    window.gc35CategoryChatInbox =
        OPEN_CATEGORY_CHAT_INBOX;
    ...
    function createCategoryChatInboxButton() {
    ...
    /* =========================================================
       INJECT BUTTON INTO CATEGORY SUBSECTION
    ========================================================= */

    function alreadyHasChatInbox(container) {
        ...
        for (var i = 0; i < buttons.length; i++) {
            ...
            if (
                text.indexOf('chat inbox') !== -1 ||
                /openCategoryChatInbox|openChatInbox|showSubPage\('messages'\)|switchSection\('messages'\)/.test(onclick)
            ) {
                return true;
            }
        }
    ...
    function injectCategoryChatInboxInto(container) {
    ...
    function injectCategoryChatInbox() {

        /*
          Category detail subsections used by h32CatTab.
          If you only want it in Members, keep only:
          'h32c-members'
        */
        var categoryPanels = [
            'h32c-members',
        ...
        /*
          Group > Categories subsection, if present.
        */
        injectCategoryChatInboxInto(
            document.getElementById(
                'gg-tab-categories'
        ...
        /*
          Fallback: visible category-like panel.
        */
        var possiblePanels =
            Array.prototype.slice.call(
                document.querySelectorAll(
                    '[id*="h32c-"], [id*="category"], [id*="categories"]'
                )
            );

        for (var i = 0; i < possiblePanels.length; i++) {
            ...
            if (isVisible) {
                injectCategoryChatInboxInto(panel);
                break;
            }
        }
    }
    ...
    /* =========================================================
       HOOK CATEGORY TAB CHANGES
    ========================================================= */

    function wrapCategoryTabForChatInbox() {

        if (
            window.__categoryChatInboxInstalled
        ) {
            return;
        }
    ...
    function installCategoryChatInbox() {

        wrapCategoryTabForChatInbox();

        injectCategoryChatInbox();

        /*
          Re-inject when category content is redrawn.
        */
        ...
        /*
          Safety interval for async feed/member loading.
        */
        if (window.__categoryChatInboxTimer) {
            clearInterval(
                window.__categoryChatInboxTimer
            );
        }
